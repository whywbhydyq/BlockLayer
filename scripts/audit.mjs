...
for (const needle of ['usePathname', 'adsense-auto-ads', 'pagead2.googlesyndication.com', 'adsbygoogle.js', 'isAdEligiblePath', 'google-auto-placed', 'removeAutoAds']) {
...
for (const allowedPath of ['/', '/minecraft-circle-generator', '/minecraft-oval-generator', '/minecraft-sphere-generator', '/minecraft-dome-generator', '/minecraft-block-count-calculator']) {
  if (!publicPolicy.includes(`'${allowedPath}'`)) failures.push(`AdSense allowlist missing ${allowedPath}`);
}
if (!publicPolicy.includes("pathname.startsWith('/presets/')")) failures.push('Preset path policy missing');
const presetRoute = read('src/app/presets/[slug]/page.tsx');
if (!presetRoute.includes('robots: { index: false, follow: true }')) failures.push('Preset detail routes must be noindex, follow');
const sitemapSource = read('src/app/sitemap.ts');
if (!sitemapSource.includes('indexableContentPaths')) failures.push('Sitemap must use the approved indexable path inventory');
if (sitemapSource.includes('allContentPaths')) failures.push('Sitemap must not publish preset and alias inventories');
