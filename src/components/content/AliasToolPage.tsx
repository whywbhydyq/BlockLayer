...
export function AliasToolPage({ eyebrow, heading, description, shape, diameter, initialDiameter, initialWidth, initialHeight, path }: AliasToolPageProps) {
  ...
  return (
    <main id="main" className="page-wrap with-sidebar">
      <JsonLd data={softwareApplicationSchema({ path: canonicalPath, shape, title: heading, heading, description, diameter: resolvedDiameter })} />
      <ToolShell initialShape={shape} initialDiameter={resolvedDiameter} initialWidth={initialWidth} initialHeight={initialHeight} title={heading} />
      <Breadcrumbs items={[{ name: 'Home', path: '/' }, { name: heading, path: canonicalPath }]} />
