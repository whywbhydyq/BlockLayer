...
import type { BuildDirection, DomeHalf, InputMode, SolidMode } from '@/lib/geometry';
import type { FormState, UpdateFormState } from './controlTypes';
import { parseNumber } from './controlTypes';

export function DomeControls({ state, update }: { state: FormState; update: UpdateFormState }) {
  if (state.shape !== 'dome') return null;
  return (
    <>
