  ...
  return (
    <>
      <div className="control-row">
        ...
        {state.inputMode === 'diameter' ? <label>Diameter blocks<input type="number" onWheel={(event) => event.currentTarget.blur()} min={1} max={1024} value={state.diameter} onChange={(event) => update('diameter', parseNumber(event.target.value, 31))} /></label> : <label>Radius blocks<input type="number" onWheel={(event) => event.currentTarget.blur()} min={1} max={512} value={state.radius} onChange={(event) => update('radius', parseNumber(event.target.value, 15))} /></label>}
      </div>
      <div className="control-row">
